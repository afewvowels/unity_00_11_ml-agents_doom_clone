...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        isOrthographic = true;
        activeIndex = 0;
        foreach (GameObject scene in GameObject.FindGameObjectsWithTag("doomscene"))
        {
            scenes.Add(scene);
        }
        GetActiveAgent();
    ...
    private void GetActiveAgent()
    {
        for (int i = 0; i < scenes[activeIndex].transform.childCount; i++)
        {
            if (scenes[activeIndex].transform.GetChild(i).CompareTag("doomagent"))
            {
                activeAgent = scenes[activeIndex].transform.GetChild(i).GetComponent<DoomAgent>();
                return;
            }
        }
    }
    ...
    {
        foreach (GameObject mazeController in GameObject.FindGameObjectsWithTag("doomcontroller"))
        {
